...
def calcular_señales_salida_v12(pnl_pct: float, precio_compra: float,
    ...
    Trailing: stop = precio_max × 0.95 (-5% desde máximo)
        ...
        for tk in _universo:
            try:
                ...
                etapa = clasificar_etapa_v12(
                    ...
                    cat_fecha="—"  # backtesting: sin earnings históricos
                )
...
def get_decision(score, pre_move, pre_vol, macd, rsi, lider):
    ...
    # v14.1 FIX: umbral 80→55 — contradicción RSI<35 + pre_move≥5 hacía M3 imposible
    # IONQ/AMD con RSI 42-50 + momentum fuerte ahora clasifican correctamente
...
def get_spy_filtro() -> dict:
    ...
    try:
        ...
        return {"mercado_positivo": True, "spy_ayer": 0.0,
                "spy_antes": 0.0, "cambio_pct": 0.0, "fuente": "error"}
...
with st.sidebar:
    st.markdown(f'<div style="font-size:22px;font-weight:800;color:{B}">🦅 GrekoTrader</div>'
            ...
            f'v15 · Mayo 2026 · Earnings en Fases M1/M2/M3 + Zona NBIS + Bloqueo · '
...
COLS_MAIN=["Ticker","Score_Rebote","Nivel_Rebote","Etapa_v12","Area","Precio","RSI","DD_pico","Cat_Fecha","Prob_NBIS","Motivo","Lectura"]
COLS_EXT =["Ticker","Score_Rebote","Nivel_Rebote","Etapa_v12","Area","Decision","Precio","Score","RSI","DD_pico","Cat_Fecha","Prob_NBIS","Detalle_Rebote","Motivo","Lectura"]
...
with tab2:
    ...
    if sw_result is None:
        ...
        for _, r in (_display if _display is not None else pd.DataFrame()).iterrows():
            ...
            # v14.1 FIX: momentum 8→4, vol 150→120 — M3 nunca se mostraba en cards
            if not _bloqueado_earn and r["Dias_Alcistas"] >= 3 and r["Momentum_3d"] >= 4 and vol_r >= 120:
                fase       = "🔥 ENTRAR HOY"
...
with tab3:
    ...
    render_scan_tab(
        ...
        rsi_max=35, dd_min=-10, score_min=55,
        decisions=["ENTRAR","ANTICIPAR"],
        prob_nbis_min=40,  # v12: M3 requiere Prob NBIS ≥ 40%
...
st.markdown(
    ...
    f'Versión 12 · Creado el 24 Abr 2026 · '
