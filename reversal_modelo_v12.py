"""
╔══════════════════════════════════════════════════════════════════╗
║  GREKOTRADER — 100% Automático · Sin datos en duro · v12               ║
║                                                                  ║
...
def calcular_señales_salida_v12(pnl_pct: float, precio_compra: float,
    ...
    # T2 +12% → vender 40%
    if pnl_pct >= 12.0:
        señales.insert(0, "T2 alcanzado — vender 40% adicional")
        return {
    ...
    if pnl_pct >= 8.0:
        return {
            ...
            "accion": "Vender 40% · Mover stop del remanente a breakeven",
            "urgencia": "HOY",
...
with st.sidebar:
    st.markdown(f'<div style="font-size:22px;font-weight:800;color:{B}">🦅 GrekoTrader</div>'
            ...
            f'v12 · 25 Abr 2026 · Prob NBIS + T1/T2 + Trailing + FLUJO_VALIDADO · '
    ...
    with col_btn:
        if st.button(f"🔍 Escanear {titulo}", use_container_width=True, key=f"btn_{tab_key}"):
            with st.spinner(f"Escaneando ~{len(SCAN_UNIVERSE)} tickers..."):
                resultado = scan_tab(rsi_max, dd_min, score_min, decisions, universo=SCAN_UNIVERSE[:st.session_state.get("universo_size", 300)], prob_nbis_min=prob_nbis_min)
...
""")
    ...
    if posiciones_df is not None and len(posiciones_df) > 0:
        ...
        for _,pos in posiciones_df.iterrows():
            ...
            _precio_max = max(pa, pc * 1.12)  # aproximar máximo si no disponible
            ...
            analisis = analizar_posicion(
                ...
                tipo=_tipo_pos,
                estrategia=_estrategia_pos)
            # Sobrescribir señal con v12 si es más urgente
            if analisis_v12["urgencia"] in ("AHORA","HOY"):
                analisis["señal"]   = analisis_v12["señal"]
                analisis["razon"]   = analisis_v12["accion"]
                analisis["color"]   = "#" + analisis_v12["color"]
                analisis["urgencia"]= analisis_v12["urgencia"]
